...
def recs(df, cols, columnar=False):
    ...
    for c in x.columns:
        ...
        if x[c].dtype == bool:
            x[c] = x[c].astype(int)
        if x[c].dtype == object:
            n = 400 if c in ("bi_comps", "bi_why") else 260                                   # the breakout reasons and lookalikes are sentences, not labels
            x[c] = x[c].astype(str).str.slice(0, n).replace({"nan": None, "None": None})
    if columnar:
...
def main(argv=None):
    ...
    payload = dict(
        ...
        projections=recs(kv, ["mlbam_id", "BI", "bi_line", "bi_why", "bi_skills", "bi_comps", "proj_rate_raw", "track_rate", "age_step", "proj_rate", "proj_PA", "durability", "il_days_w", "proj_pts", "proj_pts_600", "proj_rank",
